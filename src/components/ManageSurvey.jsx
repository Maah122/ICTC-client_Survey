...
const deleteSurvey = async (surveyId) => {
  try {
      const response = await axios.delete(`${process.env.REACT_APP_API_URL}/api/surveys/delete/${surveyId}`);
  ...
  const handleStatusChange = async (surveyId, currentStatus) => {
    ...
    if (newStatus) {
      if (activeSurveyId !== null && activeSurveyId !== surveyId) {
        await axios.put(`${process.env.REACT_APP_API_URL}/api/surveys/${activeSurveyId}/status`, { status: false });
      }
    ...
    try {
      const response = await axios.put(`${process.env.REACT_APP_API_URL}/api/surveys/${surveyId}/status`, { status: newStatus });
